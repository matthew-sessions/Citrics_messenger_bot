...
def EducationData(citydata):
    ...
    for i in citydata:
        ...
        format_text = f"🏫 {bailang}% of people in {cityname} can speak more than one language. {englishonly}% can only speak English, {other}% of people natively speak a language other than English, and {noenglish}% of people cannot speak English. \n\n"
        text = text + format_text
...
def GPS(citydata):
    ...
    for i in citydata:
        ...
        send = {
            ...
            "payload":{
                "template_type":"button",
                "text":f"Click below to view {name} on Google Maps 📍",
                "buttons":[
                {
                    "type":"web_url",
